...
TEMPJOB = r"""
jobid=$(sbatch --parsable --time=5:0 --job-name={job_name} --wrap=hostname --clusters={cluster} \
...
class SbatchForbiddenCPUOptions(SlurmSofiaBaseLocal):
    ...
    def set_executable(self):
        self.executable = TEMPJOB.format(
            ...
            slurm_partition=self.gpu_partition,
            job_name=self.__class__.__name__,
            extra=f"--gpus-per-node=3 {self.job_opts}")
...
class SbatchCorrectCPUsPerGPU(SlurmSofiaBaseLocal):
    ...
    def assert_allocation(self):
        ...
        return sn.all([
            ...
            sn.assert_found(msg, self.stdout, f'error message should contain: {msg}')
        ])
...
class SbatchForbiddenCombination(SlurmSofiaBaseLocal):
    ...
    job_opts = parameter([
        ...
        f'--gpus-per-task=1 --cpus-per-task=2 --gpus-per-node={gpus}',
        f'--gpus={gpus}',
    ])
